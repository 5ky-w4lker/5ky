/* Ass.No:
   Simulation of Non Preemptive Priority CPU Scheduling Algorithm
   Roll No:
*/
#include<stdio.h>
#include<string.h>
...
int n,i,currtime,front=-1,rear=-1,gi=0;
...
{ printf("\nHow many processes: "); scanf("%d",&n);
  printf("\nPID\tCPUB\tA.TIME\tPRIORITY\n");
  for (i=0; i<n; i++)
  { printf("%d\t",i);
	scanf("%d%d%d",&proc[i].cpub,&proc[i].atime,&proc[i].prio);
	proc[i].inrq=0;
   }
...
{ int p;
 ...
 { //add arrived processes for exec in ready queue
  ...
  //select highest prio process for execution
   p=sel_proc();
  if (p!=-1)
   execute_proc(p);
  else
...
{ int p;
 ...
 if (first==NULL)
  ...
  while (curr!=NULL)
  {	if (proc[curr->pid].prio < proc[selp->pid].prio)
	 { pselp=prev;
	   selp=curr;
	}
	prev=curr;
...
{ //process p started
  //find wtime
  wtime[p]+=(currtime-proc[p].atime);
  gchart[gi].stime=currtime;
  gchart[gi].pid=p;
  //process p finishes
  proc[p].inrq=0;
  currtime+=proc[p].cpub;
  proc[p].cpub=0;
