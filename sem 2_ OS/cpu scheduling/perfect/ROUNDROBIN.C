...
void print_gchart();
void find_times();
void remove_proc_from_rq();
void move_proc_toend_rq();
getetime(int p);
void add_proc_inrq();
...
{ int p;
 ...
 {
  //select first process from rq for execution
   p=sel_proc();
  if (p!=-1)
   execute_proc(p);
  else
...
{ //process p started
  ...
  {  remove_proc_from_rq(); proc[p].inrq=0;
...
void remove_proc_from_rq()
{
	curr=first; first=first->link;
	free(curr);
 }

void move_proc_toend_rq()
{
   if (first->link!=NULL)  //rq contains >1 number of procs
   {
	 last=first;
	  while(last->link!=NULL) last=last->link;
	 curr=first; first=first->link;
	  last->link=curr; curr->link=NULL;
   }
}
